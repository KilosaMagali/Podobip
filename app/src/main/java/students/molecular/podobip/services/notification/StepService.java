 ...
 */
public class StepService implements IStepService, SensorEventListener {

    private static String TAG = StepService.class.getSimpleName();
    private HashMap<StepListener, Integer> listeners = new HashMap<>();
    ...
    private void notifyListeners() {
        for (Map.Entry<StepListener, Integer> entryset : listeners.entrySet()) {
            if (nbSteps % entryset.getValue() == 0)
                entryset.getKey().onStepEvent();
        }
    }
